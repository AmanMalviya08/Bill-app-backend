...
exports.createInvoice = async (req, res) => {
  try {
    ...
    const branch = await Branch.findOne({ _id: branchId, company: companyId, deletedAt: null }).lean();
    ...
    const client = await Client.findOne({ _id: data.client, company: companyId, deletedAt: null }).lean();
    if (!client) return res.status(404).json({ message: 'Client not found' });

    // Generate invoice number
    const count = await Invoice.countDocuments({ branch: branchId });
